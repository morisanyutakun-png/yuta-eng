...
export const metadata: Metadata = createPageMetadata({
  title: "ノビットのしくみ｜教材 × 習慣化 × 添削で続く学習システム",
  description:
    "ノビットスタディ 中高部のしくみを詳しく解説。塾でも参考書でもない「続く学習システム」。自作教材を好きなタイミングで1枚ずつ進め、提出と同時に解答解説PDFと次の範囲、先生の添削・再提出は並行。3本柱（教材・習慣化・添削）と学習サイクルを紹介します。",
...
const pillars = [
  ...
  },
  {
    ...
    points: ["途中式・考え方まで", "減点ポイントを可視化", "運用日程に沿って返却"],
  },
];
...
const correction = [
  ...
  { mark: "③", title: "「見てくれている」実感", body: "提出を続けるほど、頑張りが伝わる。認めてもらえる一言が、次の範囲へのやる気になります。" },
  { mark: "④", title: "次まで、いっしょに", body: "返却・再提出・次の範囲をアプリで整理。ひとりで抱え込まず、必要な順番で積み上げられます。" },
];
        ...
        <Container className="relative px-6">
          ...
          <div className="py-10 sm:py-14">
            ...
            <p className="mt-4 max-w-2xl text-[1rem] leading-[1.95] text-[#334155]">
              ...
              提出と同時に解答解説PDFと次の範囲、先生の添削・再提出は並行して返る——
...
アプリの中を、ちょっと見てみる。
            ...
            <p className="mt-3 text-[0.96rem] leading-[1.95] text-[#475569]">
              スマホ・タブレットで完結。今日の範囲、PDF提出、返却・再提出、成績記録——実際の画面はこう見えます。
            </p>
          </div>
          <div className="mt-12 flex flex-wrap items-start justify-center gap-8 sm:gap-12">
            <div className="text-center">
              <AppScreen variant="home" />
              <p className="mt-4 text-[0.9rem] font-bold text-[#0b1d4a]">今日の範囲（ホーム）</p>
              <p className="mt-1 text-[0.8rem] text-[#64748b]">次の範囲・採点待ち・合格状況</p>
            </div>
            <div className="text-center">
              <AppScreen variant="returned" />
              <p className="mt-4 text-[0.9rem] font-bold text-[#0b1d4a]">添削が返る（返却）</p>
              <p className="mt-1 text-[0.8rem] text-[#64748b]">合否・得点・再提出までコメント</p>
            </div>
          </div>
      ...
      <section className="bg-white">
        <Container className="px-6 py-16 sm:py-24">
          ...
          <ol className="mx-auto mt-10 max-w-2xl space-y-3">
            {[
              { time: "20:00", title: "アプリで「今日の範囲」を開く", body: "教材ごとの現在地と次の範囲が見えます。何をやるか迷う時間はありません。" },
